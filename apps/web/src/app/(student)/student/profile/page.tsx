    ...
    <div className="max-w-4xl mx-auto px-4 py-6 sm:py-8 space-y-6">
        ...
        <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
          ...
          {!editMode ? (
            <button
              ...
              className="text-sm text-[#a2a3fc] hover:text-[#7b7cee] font-medium"
            >
              Edit
            </button>
          ) : (
            <div className="flex gap-2">
              ...
              <button
                onClick={handleSave}
                disabled={saving}
                className="flex items-center gap-1 text-sm text-[#a2a3fc] hover:text-[#7b7cee] font-medium disabled:opacity-50"
              >
        ...
        <div className="p-6 space-y-4">
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2 mt-2">
              {skills.map(skill => (
                <span key={skill} className="inline-flex items-center gap-1 px-3 py-1 bg-[#f0f0ff] text-[#a2a3fc] rounded-lg text-sm">
                  {skill}
                  {editMode && (
                    <button onClick={() => removeSkill(skill)} className="ml-1 text-[#a2a3fc] hover:text-[#7b7cee]">
                      ×
                    </button>
                  )}
                </span>
              ...
              {editMode && (
                <div className="flex items-center gap-1">
                  ...
                  />
                  <button onClick={addSkill} className="p-1 text-[#a2a3fc] hover:text-[#7b7cee]">
                ...
                <div key={step.key} className="flex items-center justify-between py-2">
                  <div className="flex items-center gap-3">
                    <div className={`w-8 h-8 rounded-full flex items-center justify-center ${
                      isComplete ? 'bg-[#f0f0ff]' : 'bg-[#f5f5f5]'
                    }`}>
                      {isComplete ? (
                  ...
                  <span className={`text-xs font-medium px-2 py-0.5 rounded ${
                    isComplete ? 'bg-[#f0f0ff] text-[#a2a3fc]' :
                    status === 'in_progress' ? 'bg-[#f5f5f5] text-[#6b6b80]' :
      ...
      <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 p-6">
          ...
          <div className="text-center">
            <div className="text-2xl font-bold text-[#a2a3fc]">
